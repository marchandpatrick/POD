...
plotPOD <- function(obj, model="auto", qLOD=95, show.warnings=FALSE){
    x <- obj$x
    XLIM <- range(x$X);
    YTICK <- seq(0, 1, 0.05)
    ...
    b <- FIT$b
    LodList <- FIT$LOD
    if( ! as.character(qLOD) %in% rownames(LodList) ){
    ...
    for( nLOD in as.character(qLOD) ){
        ...
        if( LOD[1] > 1e-3 ){
            ...
            text(x=LOD[1], y=0.05, pos=4, labels=bquote(paste(LOD[.(nLOD)*'%']== .(LODr))), col=.COLOR.LOD)
        }
    }
